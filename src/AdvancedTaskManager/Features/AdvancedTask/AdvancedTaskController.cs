                        ...
                        if (data.PublishContent)
                        {
                            ...
                            if (depContent != null && _helper.CanUserPublish(depContent))
                            {
                                try
                                {
                                    ...
                                    {
                                        case PageData page:
                                            _contentRepository.Save(page.CreateWritableClone(), SaveAction.Publish, AccessLevel.Publish);
                                            break;
                                        case BlockData block:
                                            _contentRepository.Save(block.CreateWritableClone() as IContent, SaveAction.Publish, AccessLevel.Publish);
                                            break;
                                        case ImageData image:
                                            _contentRepository.Save(image.CreateWritableClone() as IContent, SaveAction.Publish, AccessLevel.Publish);
                                            break;
                                        case MediaData media:
                                            _contentRepository.Save(media.CreateWritableClone() as IContent, SaveAction.Publish, AccessLevel.Publish);
                                            break;
                                    }
                                }
                ...
                try
                {
                    ...
                    if (approval is ContentApproval contentApproval && publishContent)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                case PageData page:
                                    clone = page.CreateWritableClone();
                                    break;
                                case BlockData block:
                                    clone = block.CreateWritableClone() as IContent;
                                    break;
                                case ImageData image:
                                    clone = image.CreateWritableClone() as IContent;
                                    break;
                                case MediaData media:
                                    clone = media.CreateWritableClone() as IContent;
                                    break;
                            }
